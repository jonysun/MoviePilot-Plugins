...
class AutoDiagnosisPlus(_PluginBase):
    # 插件名称
    plugin_name = "自动诊断plus"
    # 插件描述
    plugin_desc = "自动发起系统健康检查、网络连通性测试以及硬链接检查,支持微信菜单命令。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/InfinityPacer/MoviePilot-Plugins/main/icons/autodiagnosis.png"
    # 插件版本
    plugin_version = "1.0.1"
    # 插件作者
    plugin_author = "InfinityPacer,jonysun"
    # 作者主页
    author_url = "https://github.com/InfinityPacer"
    # 插件配置项ID前缀
    plugin_config_prefix = "autodiagnosisplus_"
    # 加载顺序
    plugin_order = 40
    # 可使用的用户级别
    auth_level = 1
    ...
    def get_command() -> List[Dict[str, Any]]:
        ...
        """
        return [{
            "cmd": "/auto_diagnosis_run",
            "event": EventType.PluginAction,
            "desc": "健康检查",
            "category": "命令菜单（微信）",
            "data": {
                "action": "auto_diagnosis_run"
            }
        }]
        pass
    ...
    def __check_health_modules(self) -> List[Dict[str, Any]]:
        ...
        for module_id in selected_module_ids:
            ...
            result_state = False if not state and errmsg else True
            results.append({
                "id": module_id,
                "name": module_name,
                "state": result_state,
                "errmsg": errmsg or "",
                "result": "正常" if state else ("未启用" if not errmsg else "错误")
            })
    ...
    def auto_diagnosis_run(self, event: Event):
        """
        健康检查
        """
        if event:
            event_data = event.event_data
            if not event_data or event_data.get("action") != "auto_diagnosis_run":
                return

            logger.info("收到命令，开始执行系统健康检查 ...")
        ...
        if event:
            self.post_message(channel=event.event_data.get("channel"),
                              title="系统健康检查完成！", userid=event.event_data.get("user"))