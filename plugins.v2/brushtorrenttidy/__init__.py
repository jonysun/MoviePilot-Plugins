...
class BrushTorrentTidy(_PluginBase):
    # 插件名称
    plugin_name = "刷流种子整理(自用修改版)"
    # 插件描述
    plugin_desc = "针对刷流种子进行整理操作，转移入库。"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "brushtorrenttidy_"
    # 加载顺序
    plugin_order = 23
    # 可使用的用户级别
    auth_level = 2
    ...
    def __get_plugin_options(self) -> List[dict]:
        ...
        # 需要检查的插件名称
        filter_plugins = {"BrushFlow", "BrushFlowLowFreq", "BrushFlowPlus"}
    ...
    def __get_all_hashes_and_torrents(self, torrents):
        ...
        try:
            ...
            for torrent in torrents:
                # 根据下载器类型获取Hash值
                hash_value = torrent.get("hash") if is_qbittorrent else torrent.hashString
