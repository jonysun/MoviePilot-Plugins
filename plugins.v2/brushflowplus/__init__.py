...
class BrushFlowPlus(_PluginBase):
    ...
    # 作者主页
    author_url = "https://github.com/jonysun/MoviePilot-Plugins"
    # 插件配置项ID前缀
    plugin_config_prefix = "brushflowplus"
    # 加载顺序
    plugin_order = 22
    # 可使用的用户级别
    auth_level = 2
    ...
    def get_dashboard(self, key: str, **kwargs) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], List[dict]]]:
        ...
        elements = [
            # 第一行：汇总元素卡片
            {
                'component': 'VRow',
                'props': {
                'justify': 'center' # 使 5 个 VCol 在行内居中
            },
    ...
    def __brush_site_torrents(self, siteid, torrent_tasks: Dict[str, dict], statistic_info: Dict[str, int],
        ...
        for torrent in torrents:
            ...
            site_size_condition_passed, reason = self.__evaluate_site_size_condition_for_brush(
                site_sizes=site_sizes, siteid=siteid, add_torrent_size=torrent.size)
    ...
    def __evaluate_conditions_for_brush(self, torrent, torrent_tasks) -> Tuple[bool, Optional[str]]:
        ...
        # 下载人数
        if brush_config.peers:
            peers_range = [float(n) for n in brush_config.peers.split("-")]
            # 检查是否仅指定了一个数字，即做种人数需要小于等于该数字
            if len(peers_range) == 1:
                # 当做种人数大于该数字时，不符合条件
                if torrent.peers > peers_range[0]:
                    return False, f"下载人数 {torrent.peers}，超过单个指定值"
            # 如果指定了一个范围
            elif len(peers_range) > 1:
                # 检查做种人数是否在指定的范围内（包括边界）
                if not (peers_range[0] <= torrent.peers <= peers_range[1]):
                    return False, f"做种人数 {torrent.peers}，不在指定范围内"
    ...
    def __delete_torrent_for_evaluate_conditions(self, torrents: List[Any], torrent_tasks: Dict[str, dict],
        ...
        for torrent in torrents:
            ...
            ratio = torrent_task.get("ratio", 0)
            torrent_info = self.__get_torrent_info(torrent)
            ...
            if should_delete:
                ...
                self.__send_delete_message(site_name=site_name, torrent_title=torrent_title, torrent_desc=torrent_desc,ratio=ratio,
                                           reason=reason)
                ...
                if seeding_time:
                    ...
                    if not proxy_size_range:
                        self.__send_delete_message(site_name=site_name, torrent_title=torrent_title,
                                                   torrent_desc=torrent_desc,ratio=ratio,
                                                   reason=reason)
                    logger.info(f"站点：{site_name}，{reason}，删除种子：{torrent_title}|{torrent_desc}")
    ...
    def __send_delete_message(self, site_name: str, torrent_title: str, torrent_desc: str, reason: str, ratio: float = 0.0,
                              title: str = "【刷流任务种子删除】"):
        """
        发送删除种子的消息
        """
